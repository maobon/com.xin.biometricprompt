...
public class KeyAttestationExample {
    ...
    public static void testtest(X509Certificate certs) throws Exception {
        // X509Certificate[] certs = loadCertificates();
        // verifyCertificateChain(certs);

        // Get the attestation extension data as an ASN.1 SEQUENCE.
        ASN1Sequence extensionData = extractAttestationSequence(certs);
        ...
        int attestationVersion = getIntegerFromAsn1(extensionData.getObjectAt(ATTESTATION_VERSION_INDEX));
        ...
        // whether the attestation certificate was generated in that TEE.

        // In this example, the test attestation certificate was generated on a device with a TEE, but the certificate was generated in software.
        ...
        byte[] attestationChallenge = ((ASN1OctetString) extensionData.getObjectAt(ATTESTATION_CHALLENGE_INDEX)).getOctets();
        ...
        ASN1Encodable[] softwareEnforced = ((ASN1Sequence) extensionData.getObjectAt(SW_ENFORCED_INDEX)).toArray();
        ASN1Encodable[] teeEnforced = ((ASN1Sequence) extensionData.getObjectAt(TEE_ENFORCED_INDEX)).toArray();

        // Since this key is TEE-based, we expect to find the key size in the TEE list.
        ...
        if (purposes == null || purposes.toArray().length != 1 || getIntegerFromAsn1(purposes.toArray()[0]) != KM_PURPOSE_SIGN) {
            throw new Exception("This key does not have the expected purpose.");
        }

        // We expect this key to be authorized for use only if the user has authenticated within the last 300 seconds (5 minutes).
        int authTimeout = getIntegerFromAsn1(findAuthorizationListEntry(teeEnforced, KM_TAG_AUTH_TIMEOUT));
        ...
        // The user can authenticate with either a password or a fingerprint to authorize this key for use.
        int authTypes = getIntegerFromAsn1(findAuthorizationListEntry(teeEnforced, KM_TAG_USER_AUTH_TYPE));
        ...
        // property.
        boolean rollbackResistant = (null != findAuthorizationListEntry(teeEnforced, KM_TAG_ROLLBACK_RESISTANT));
    ...
    private static void verifyCertificateChain(X509Certificate[] certs)
            throws CertificateExpiredException, CertificateNotYetValidException, CertificateException,
        ...
        X509Certificate secureRoot = (X509Certificate) CertificateFactory.getInstance("X.509").generateCertificate(new ByteArrayInputStream(GOOGLE_ROOT_CERTIFICATE.getBytes()));
...
//        }
    ...
    private static X509Certificate[] loadCertificates() throws CertificateException {
        ...
        for (int i = 0; i < SAMPLE_ATTESTATION_CERT_CHAIN.length; ++i) {
            certs[i] = (X509Certificate) factory.generateCertificate(new ByteArrayInputStream(SAMPLE_ATTESTATION_CERT_CHAIN[i].getBytes()));
        }
        return certs;
    }
